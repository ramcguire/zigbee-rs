...
def wire_case(name: str, wire: bytes) -> JsonObject:
    return named_wire(name, wire)


def type_mismatch_case(
    name: str, wire: bytes, *, expected: int, found: int
) -> JsonObject:
...
def invalid_case(
    ...
    *,
    ...
    fields: JsonObject = {"error": error}
    if expected is not None:
        fields["expected"] = expected
    if found is not None:
        fields["found"] = found
    return named_wire(name, wire, **fields)
...
def gen_int(zigpy_type: type, bits: int, *, signed: bool) -> JsonObject:
    ...
    return fixture(
        ...
        null_wire=[wire_case("null", serialize(null_value))],
    )
...
def gen_bool() -> JsonObject:
    ...
    return fixture(
        ...
        null_wire=[wire_case("null", bytes([0xFF]))],
        invalid_value=[
            wire_case("reserved_low", bytes([0x02])),
            wire_case("reserved_high", bytes([0xFE])),
        ],
    )
...
def gen_float(zigpy_type: type, width: int) -> JsonObject:
    ...
    return fixture(
        ...
        null_wire=[wire_case("canonical_nan_null", serialize(float("nan")))],
    )
...
def counted_bytes(value: object) -> bytes:
    if isinstance(value, str):
        ...
        raise TypeError(f"counted_bytes needs str or bytes, got {type(value).__name__}")
...
def gen_short_text() -> JsonObject:
    ...
    return fixture(
        ...
        null_wire=[wire_case("null", bytes([0xFF]))],
        invalid_utf8=[wire_case("invalid_utf8", bytes([0x03, 0xFF, 0xFE, 0xFD]))],
    )
...
def collection_case(
    ...
    return values_case(
        name,
        zigpy_collection(collection_type, elem_type_id, zigpy_type, values),
        values,
    )
...
def collection_error_sections(
    ...
    return {
        "null_wire": [wire_case("null", encode_coll_null(elem_type_id))],
        "type_mismatch": [
            type_mismatch_case(
                "wrong_element_type",
...
def gen_collection(
    ...
    return fixture(
        roundtrip=[
            collection_case(name, collection_type, elem_type_id, zigpy_type, values)
...
def gen_collection_set(
    ...
    return fixture(
        valid=[
            collection_case(name, f.Set, elem_type_id, zigpy_type, values)
...
def encode_pair_struct(u8_val: int, u16_val: int) -> bytes:
    return pair_struct(u8_val, u16_val).serialize()
...
def gen_collection_struct_pair() -> JsonObject:
    ...
    return fixture(
        roundtrip=[
            value_case(name, encode_pair_struct(u8_val, u16_val), [u8_val, u16_val])
            for name, u8_val, u16_val in samples
        ],
        null_wire=[wire_case("null", struct_header(0xFFFF))],
        invalid=[
            invalid_case(
...
def gen_nested_array_u8() -> JsonObject:
    ...
    return fixture(
        roundtrip=roundtrip,
        null_wire=[wire_case("null", encode_coll_null(array_type_id))],
        type_mismatch=[
            type_mismatch_case(
                "wrong_element_type",
...
def gen_array_of_struct_pair() -> JsonObject:
    ...
    return fixture(
        roundtrip=roundtrip,
        null_wire=[wire_case("null", encode_coll_null(struct_type_id))],
        type_mismatch=[
            type_mismatch_case(
                "wrong_element_type",
        ...
        invalid=[
            ...
            invalid_case(
                "trailing_payload",
                encode_coll(
                    struct_type_id, encode_pair_struct(0x42, 0x1234) + bytes([0x00]), 1
                ),
                "InvalidLength",
            ),
        ],
...
def int_generator(data_type: f.DataTypeId, zigpy_type: type, bits: int, *, signed: bool) -> Generator:
...
GENERATORS: dict[str, Generator] = {
    ...
    "enum8": generator(
        ...
        lambda: gen_enum(
            t.enum8,
            [("zero", 0x00), ("one", 0x01), ("arbitrary", 0x42), ("max_non_null", 0xFE)],
            0xFF,
        ),
    ),
    "enum16": generator(
    ...
    "coll_bag_u8": collection_generator(
        ...
        1,
        t.uint8_t,
        [("empty", []), ("single", [42]), ("multiple", [1, 100, 0xFE]), ("duplicates_allowed", [1, 1, 2])],
        TYPE_U16,
    ),
    "coll_bag_u16": collection_generator(
        ...
        2,
        t.uint16_t,
        [("empty", []), ("single", [1]), ("multiple", [1, 2, 3]), ("duplicates_allowed", [1, 1, 2])],
        TYPE_U8,
    ),
