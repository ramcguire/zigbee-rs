...
impl ClusterServer for BasicServer {
    ...
    fn check_write_attribute(
        ...
        match id.0 {
            ...
            0x0000 | 0x0004 | 0x0005 | 0x0007 | 0xFFFD => Err(AttrError::ReadOnly),
            _ => Err(AttrError::UnsupportedAttribute),
        }
    }
    ...
    fn snapshot(&self, buf: &mut [u8]) -> usize {
        self.device_enabled.snapshot(buf)
    }

    fn restore_snapshot(&mut self, buf: &[u8]) {
        self.device_enabled.restore_snapshot(buf);
    }
}
...
mod tests {
    ...
    fn attribute_list_includes_basic_attrs() {
        ...
        assert_eq!(attrs[5].id, AttributeId::new(0xFFFD));
        assert_eq!(attrs[5].type_id, TypeId::Uint16);
    }
    ...
    fn discover_attributes_lists_basic_attrs() {
        ...
        assert_eq!(&buf[19..22], &[0xFD, 0xFF, TypeId::Uint16.as_u8()]);
    }
}
