mod helpers;
mod vectors {
    include!("../vectors/clusters/flow.rs");
}

use helpers::assert_response;
use helpers::dispatch_bytes;
use zigbee_cluster_library::measurement::flow::FlowMeasurementServer;

// READ_CASES[0..=2]: measured_value=[0, 100, 65534]
// READ_CASES[3]: min_measured_value=0
// READ_CASES[4]: max_measured_value=1000
// READ_CASES[5]: tolerance=100
fn make_read_server(i: usize) -> FlowMeasurementServer {
    let mut s = FlowMeasurementServer::new();
    match i {
        0 => s.set_measured_value(Some(0)).unwrap(),
        1 => s.set_measured_value(Some(100)).unwrap(),
        2 => s.set_measured_value(Some(65534)).unwrap(),
        3 => s.set_min_measured_value(Some(0)).unwrap(),
        4 => s.set_max_measured_value(Some(1000)).unwrap(),
